<template>
  <div>
    ...
    <el-form :inline="true"   class="demo-form-inline">
      <el-form-item>
        <el-button type="primary" @click="dialogFormVisible = true">添加内容</el-button>
      </el-form-item>
      <el-form-item  >
      ...
      <el-form :model="contentForm" :rules="rules" ref="contentForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="用户头像">
          <el-upload
            ...
            <img v-if="contentForm.customer_image" :src="contentForm.customer_image" class="avatar">
    ...
    <el-dialog title="修改内容" :visible.sync="dialogFormVisible2">
      <el-form :model="contentForm" :rules="rules" ref="contentForm" label-width="100px" class="demo-ruleForm">
    ...
    <el-table
      ...
      <el-table-column label="用户头像"  min-width="12%" prop="image">

        <template width="12%" slot-scope="scope">

          <img style="width:80px;height:80px;border:none;" :src="scope.row.customer_image">

        </template>

      </el-table-column>

      <el-table-column label="图片"  min-width="12%" prop="photos">
      ...
      <el-table-column
        ...
        label="标签"
        min-width="12%">
        <template slot-scope="scope">{{options[scope.row.tag]['label']}}</template>
      </el-table-column>
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button @click="deleteContent(scope.row)" type="text" size="small">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    deleteContent(row){
      ...
      this.$confirm('确定删除该数据?','提示',{
        ...
        axios.get('http://localhost:7777/content/deleteById/'+row.id).then(function (resp){
    ...
    edit(row) {
      const _this = this
      _this.rowid = row.id;
      axios.get('http://localhost:7777/content/findById/'+ _this.rowid).then(function (resp){
    ...
    select(name){
      const _this = this
      if (name !== "") {
        axios.get('http://localhost:7777/content/findByName?name=' + name).then(function (resp) {
          console.log(resp)
    ...
    page(currentPage){
      const _this = this
      axios.get('http://localhost:7777/content/getContent/'+currentPage+'/5').then(function (resp){
    ...
    handleAvatarSuccess2(response, file) {
      console.log(file.name)
      console.log("图片路径=" + response.returnFileUrl)
      this.contentForm.customer_image = response.returnFileUrl
  ...
  created() {
    const _this = this
    axios.get('http://localhost:7777/content/getContent/1/5').then(function (resp){
      _this.tableData = resp.data.result.records
