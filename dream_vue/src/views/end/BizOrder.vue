<template>
  <div>
    ...
    <el-dialog title="增加订单" :visible.sync="dialogFormVisible">
      <el-form :model="bizOrderForm" :rules="rules" ref="bizOrderForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="客户名称" prop="customer_name">
          <el-input v-model="bizOrderForm.customer_name"  style="width: 250px"></el-input>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitForm('bizOrderForm')">立即创建</el-button>
    ...
    <el-dialog title="修改用户" :visible.sync="dialogFormVisible2">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="resetForm('bizOrderForm')">重置</el-button>
        <el-button type="primary" @click="editForm('bizOrderForm')">立即修改</el-button>
    ...
    <el-table
      ...
      <el-table-column label="意向图片"  min-width="12%" prop="image">
...
export default {
  ...
  methods: {
    ...
    deleteUser(row){
      ...
      this.$confirm('确定删除该数据?','提示',{
        ...
        axios.get('http://localhost:7777/order/deleteById/'+row.id).then(function (resp){
    ...
    edit(row) {
      const _this = this
      _this.rowid = row.id;
      axios.get('http://localhost:7777/order/findById/'+ _this.rowid).then(function (resp){
    ...
    select(name){
      const _this = this
      if (name !== "") {
        axios.get('http://localhost:7777/order/findByName?name=' + name).then(function (resp) {
          console.log(resp)
    ...
    page(currentPage){
      const _this = this
      axios.get('http://localhost:7777/order/getOrder/'+currentPage+'/5').then(function (resp){
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('http://localhost:7777/order/add',this.bizOrderForm).then(function (resp){
  ...
  created() {
    const _this = this
    axios.get('http://localhost:7777/order/getOrder/1/5').then(function (resp){
      _this.tableData = resp.data.result.records
